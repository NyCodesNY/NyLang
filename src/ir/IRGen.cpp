...
void IRGen::genStatement(const Statement& stmt) {
    ...
    if (auto* s = dynamic_cast<const ForStatement*>(&stmt))        { genFor(*s); return; }
...
void IRGen::genFor(const ForStatement& forStmt) {
    if (forStmt.init) {
    ...
    if (forStmt.condition) {
        IRValue cond = genExpr(*forStmt.condition);
        emit(IRInstr::makeJumpIfZero(cond, endLabel));
    }
    ...
    if (forStmt.increment) {
        genStatement(*forStmt.increment);
    }
