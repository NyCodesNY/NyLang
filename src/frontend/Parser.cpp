...
std::unique_ptr<ForStatement> Parser::parseForStatement() {
    ...
    expect(TokenType::LParen, "for condition");
    advance();

    std::unique_ptr<Statement> init = nullptr;
    if (current().type != TokenType::Semicolon) {
        init = parseStatement(); // consumes ;
    } else {
        advance(); // consume ;
    }
    ...
    expect(TokenType::Semicolon, "for condition end");
    advance();
    ...
    if (current().type != TokenType::RParen) {
        if (current().type == TokenType::Identifier || current().type == TokenType::Star) {
            auto expr = parseExpression();
            expect(TokenType::Eq, "assignment in for loop increment");
            advance();
            auto right = parseExpression();
            increment = std::make_unique<AssignmentStatement>(std::move(expr), std::move(right));
        } else {
            throw std::runtime_error("Expected assignment in for loop increment");
        }
    }

    expect(TokenType::RParen, "for loop header end");
    advance();
